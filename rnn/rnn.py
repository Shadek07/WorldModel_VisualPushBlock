...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    INWIDTH = hps.input_seq_width # 65 channels
    OUTWIDTH = hps.output_seq_width # 64 channels
    ...
    self.input_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, INWIDTH]) #100x1000x65
    self.output_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, OUTWIDTH]) #100x1000x64
...
def rnn_next_state(rnn, z, action, prev_state):
  ...
  input_x = np.concatenate((z.reshape((1, 1, 64)), action.reshape((1, 1, 1))), axis=2) #only one action is chosen
  feed = {rnn.input_x: input_x, rnn.initial_state:prev_state}
  return rnn.sess.run(rnn.final_state, feed)

def rnn_output_size(mode):
  if mode == MODE_ZCH:
    return (64+256+256) #64 for z_size
