...
### ES related code
num_episode = 1
eval_steps = 10 # evaluate every N_eval steps
retrain_mode = True
cap_time_mode = True
...
def receive_packets_from_slaves():
  ...
  for i in range(1, num_worker+1):
    ...
    for result in results:
      ...
      idx = int(result[1])
      reward_list_total[idx, 0] = result[2] #reward
      reward_list_total[idx, 1] = result[3] #time
      check_results[idx] = 0
...
def evaluate_batch(model_params, max_len=-1):
  ...
  for i in range(es.popsize):
    solutions.append(np.copy(model_params)) #same params for all agents/processes
  ...
  return np.mean(reward_list) #mean is calculated combining results from all processess

def slave():
  #model.make_env()
  model.make_env(render_mode=True, full_episode=True, worker_id=rank)  # worker_id=int(sys.argv[1])
...
def master():
  ...
  model.make_env(render_mode=False, full_episode=True, worker_id=rank) # worker_id=int(sys.argv[1])
  ...
  while True:
    ...
    solutions = es.ask() #deliver (one ore more) candidate solution(s) in a list
    ...
    es.tell(reward_list) #update internal state, prepare for next iteration
...
if __name__ == "__main__":
  ...
  parser.add_argument('--num_episode', type=int, default=5, help='num episodes per trial')
  parser.add_argument('--eval_steps', type=int, default=5, help='evaluate every eval_steps step')
  parser.add_argument('-n', '--num_worker', type=int, default=24) #2 threads
